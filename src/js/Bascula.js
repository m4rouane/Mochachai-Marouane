class Bascula {
    ...
    anotarPeso(peso, altura = 1.0, fecha = new Date()) {
        ...
        if (isNaN(peso) || isNaN(altura)) {
            throw new Error("El valor del peso y la altura no son válidos, deben ser números válidos");
        }
    ...
    obtenerPesoMaximo() {
        if (this.registros.length === 0) {
            throw new Error("No hay registros para calcular el peso máximo");
        }
        const pesos = this.registros.map(registro => registro.peso);
    ...
    obtenerPesoMinio() {
        if (this.registros.length === 0) {
            throw new Error("No hay registros para calcular el peso mínimo.");
        }
        const pesos = this.registros.map(registro => registro.peso);
    ...
    describirIMC(imc) {
        if (isNaN(imc)) {
            throw new Error("El valor de IMC no es válido, introduzca un valor correcto");
        }
        if (imc < 16) {
...
try {
    ...
    const numAnotaciones = bascula.obtenerNumeroAnotacones();
